...
The learning rate provides a rather smooth convergence of the loss at a steady rate. A larger learning rate might lead to overshooting of the local minima a much lower learning rate may take very long time to converge to the local minima. This needs to be set at the optimum value and ideally for the initial epochs the value could be higher for a faster convergence and as the loss value reduces, for the later epochs, the learning rate can be reduced to avoid overshooting the local minima.
